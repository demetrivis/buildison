...
## ⚠️ Duas armadilhas que falham do jeito errado

Estas não avisam. Uma abre o que devia fechar; a outra derruba o que devia continuar de pé.

### `basicauth.users` sem `usuario:` deixa a rota PÚBLICA

O `openssl passwd -apr1` devolve **só o hash**. O Traefik exige `usuario:hash`:

```
# ERRADO — o Traefik recusa e DESABILITA o router inteiro
- traefik.http.middlewares.x-auth.basicauth.users=$$apr1$$abc$$def

# CERTO
- traefik.http.middlewares.x-auth.basicauth.users=admin:$$apr1$$abc$$def
```

O perigo não é o erro, é o que vem depois: com o router desabilitado, o tráfego **não falha** — cai
no router menos específico daquele host, que normalmente não tem auth. A página responde **200 pra
qualquer um**, e nada na resposta indica que a proteção caiu.

O único sinal está no log do Traefik e na API dele:

```bash
curl -s -u "$USER:$PASS" https://<dashboard>/api/http/routers \
  | grep -A2 '"status":"disabled"'
```

**Depois de criar qualquer router com auth, bata sem credencial e exija 401.** Não confie no
"convergiu" — ele mede o serviço, não a proteção.

### `replicas: 1` sem `order: start-first` dá 404 em todo deploy

O padrão do Swarm é `stop-first`: a task velha morre **antes** de a nova subir. Com uma réplica só,
o Traefik fica sem backend por alguns segundos e responde 404. O sintoma engana — parece rota
inexistente, e é ausência de destino.

```yaml
    deploy:
      replicas: 1
      update_config:
        order: start-first      # sobe a nova antes de derrubar a velha
        failure_action: rollback
```

Serviço sem estado não paga nada por rodar duas instâncias num instante.
