...
<!-- Gerado de .claude/agents/infra.md por gen-antigravity.mjs — não edite à mão. -->
...
Before writing any code, read the infra skill and its references for project conventions — in the project's `.agents/skills/`, or in `~/.gemini/config/skills/` if buildison is installed globally:
- `.agents/skills/infra/SKILL.md`
- `.agents/skills/infra/references/` (docker, env-config, adr-template)
