...
<!-- Gerado de .claude/agents/suporte.md por gen-antigravity.mjs — não edite à mão. -->
...
- **Mode switch** is the `qdrant-setup` skill (command `/qdrant`), which runs
  ...
  `.agents/skills/qdrant-setup/`, or `~/.gemini/config/skills/qdrant-setup/` / `~/.agents/skills/qdrant-setup/`
