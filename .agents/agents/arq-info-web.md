...
- Para localizar padrões e nomear entidades com consistência, reaproveite os **templates de
  saída** da skill `plano-operacao` (`.agents/skills/plano-operacao/SKILL.md` e
  `.agents/skills/plano-operacao/references/` — c4-conventions, data-model-mdm,
  output-templates — ou em `~/.gemini/config/skills/plano-operacao/` se o buildison estiver instalado no global).
